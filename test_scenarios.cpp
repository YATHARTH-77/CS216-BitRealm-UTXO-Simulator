...
void run_tests(UTXOManager& mgr, Mempool& pool) {
    ...
    std::cout << "[Test 1] Valid Tx: Alice -> Bob (10 BTC)... ";
    ...
    std::cout << (res1.first ? "PASS" : "FAIL") << "\n";

    // Test 3: Double Spend in Mempool
    std::cout << "[Test 3] Double Spend (Mempool): Alice -> Charlie using same UTXO... ";
